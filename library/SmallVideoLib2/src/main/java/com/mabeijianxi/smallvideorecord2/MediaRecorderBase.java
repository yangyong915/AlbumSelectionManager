...
//		TODO 设置浏览尺寸
    ...
    protected Boolean doCompress(boolean mergeFlag) {
        if (compressConfig != null) {
            ...
            String cmd_transcoding = String.format("ffmpeg -threads 32 -i %s -c:v libx264 %s %s %s -c:a libfdk_aac %s %s %s %s",
...
//            System.out.println("TANHQ===> doCompress(): all cmd string = " + cmd_transcoding);
//            System.out.println("TANHQ===> doCompress(): videoPath = " + mMediaObject.getOutputTempVideoPath());
//            System.out.println("TANHQ===> doCompress(): bitrate = " + getBitrateModeCommand(compressConfig, "", false));
//            System.out.println("TANHQ===> doCompress(): Velocity = " + getBitrateVelocity(compressConfig, "-preset:v ultrafast", false));
//            System.out.println("TANHQ===> doCompress(): vbr = " + vbr);
//            System.out.println("TANHQ===> doCompress(): getFrameRateCmd = " + getFrameRateCmd());
//            System.out.println("TANHQ===> doCompress(): scaleWH = " + scaleWH);
//            System.out.println("TANHQ===> doCompress(): temp video path = " + mMediaObject.getOutputTempTranscodingVideoPath());
    ...
    protected Boolean doCrop(String srcVideoPath, String decVideoPath, int starts, int duration) {

//        String cmdCropString = String.format("ffmpeg -ss %s -t %s -i %s -c:v libx264 -crf 28 -preset:v ultrafast -c:a libfdk_aac -vbr 4 -r 15 %s %s",
//                String.valueOf(starts),
//                String.valueOf(duration),
//                srcVideoPath,
//                "-s 540x960",
//                decVideoPath);

//        String cmdCropString = String.format("ffmpeg -ss %s -t %s -i %s -vf scale=540:960 -c:v libx264 -c:a libfdk_aac -vbr 4 %s",
//                String.valueOf(starts),
//                String.valueOf(duration),
//                srcVideoPath,
//                decVideoPath);
        ...
        String cmdCropString = String.format("ffmpeg -ss %s -t %s -i %s -vcodec copy -acodec copy %s",
                String.valueOf(starts),
                String.valueOf(duration),
                srcVideoPath,
                decVideoPath);

        //System.out.println("TANHQ===> doCrop(): cmdCropString = " + cmdCropString);
        boolean result = FFmpegBridge.jxFFmpegCMDRun(cmdCropString) == 0;
...
//        }
    ...
    protected Boolean doScale(String srcVideoPath, String decVideoPath) {

        String cmdScaleString = String.format( "ffmpeg -threads 16 -i %s -vf scale=540:960 -c:v libx264 -c:a libfdk_aac %s", srcVideoPath, decVideoPath);

        //System.out.println("TANHQ===> doScale(): cmdScaleString = " + cmdScaleString);
        boolean result = FFmpegBridge.jxFFmpegCMDRun(cmdScaleString) == 0;
